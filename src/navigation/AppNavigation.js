...
export default function AppNavigation(){
    ...
    useEffect(()=>{const auth=getAuth(); onAuthStateChanged(auth,(user)=>{ 
       ...
       return session?(
        <Tab.Navigator screenOptions={({route})=>({headerShown:false, tabBarActiveTintColor:"#00E9ED", tabBarInactiveTintColor:"#337AFF", tabBarIcon:({color,size})=>showIcons(route,color,size)})}>
        <Tab.Screen
        name="index"
        component={IndexStack}
        options={{ title: "Inicio" }}
